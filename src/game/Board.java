...
public class Board {
    ...
    public void addRandomTile() {
        ...
        BoardSpot spot = openSpaces.get(StdRandom.uniform(0, spaces));
        ...
        gameBoard[spot.getRow()][spot.getCol()] = tileValue;
    }
    ...
    public void flipRows() {
        for (int i = 0 ; i < gameBoard.length ; i++){
            for (int j = 0 ; j < gameBoard[i].length/2 ; j++){
                int val1 = gameBoard[i][j];
                int val2 = gameBoard[i][gameBoard[i].length-1-j];
                gameBoard[i][j] = val2;
                gameBoard[i][gameBoard[i].length-j-1] = val1;
            }
        }
    }
